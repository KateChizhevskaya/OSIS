...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CONTEXTMENU:
		if ((HWND)wParam == listBox1) {
			...
			if (itemId != -1) {
				...
				AppendMenu(hPopupMenu, pC == IDLE_PRIORITY_CLASS ? MF_CHECKED : MF_STRING, ID_IDLE, L"Íèçêèé");
				AppendMenu(hPopupMenu, pC == NORMAL_PRIORITY_CLASS ? MF_CHECKED : MF_STRING, ID_NORMAL, L"Íîðìàëüíûé");
				AppendMenu(hPopupMenu, pC == HIGH_PRIORITY_CLASS ? MF_CHECKED : MF_STRING, ID_HIGH, L"Âûñîêèé");
				AppendMenu(hPopupMenu, pC == REALTIME_PRIORITY_CLASS ? MF_CHECKED : MF_STRING, ID_REAL_TIME, L"Ðåàëüíîãî âðåìåíè");
	...
	case WM_PAINT:
		...
		TextOut(hdc, 105, 20, L"Ïðîöåññû:", wcslen(L"Ïðîöåññû:"));
		TextOut(hdc, 390, 20, L"Ìîäóëè:", wcslen(L"Ìîäóëè:"));
		EndPaint(hWnd, &ps);
		break;
	case WM_DESTROY:
		PostQuitMessage(0);
